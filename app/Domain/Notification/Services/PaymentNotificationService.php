    ...
    public static function notifyCreatorOfPaymentAvailable($contract): void
    {
        try {
            ...
            $notification = Notification::create([
                ...
                'message' => "O pagamento do contrato '{$contract->title}' está disponível para saque. Valor: R$ ".number_format((float) $contract->creator_amount, 2, ',', '.'),
                'data' => [
                    'contract_id' => $contract->id,
                    'contract_title' => $contract->title,
                    'amount' => $contract->creator_amount,
                    'formatted_amount' => 'R$ '.number_format((float) $contract->creator_amount, 2, ',', '.'),
                ],
                'read_at' => null,
            ]);
    ...
    public static function notifyBrandOfPaymentSuccessful($contract): void
    {
        try {
            $notification = Notification::create([
                ...
                'message' => "O pagamento do contrato '{$contract->title}' foi processado com sucesso. Valor: R$ ".number_format((float) $contract->budget, 2, ',', '.'),
                'data' => [
                    'contract_id' => $contract->id,
                    'contract_title' => $contract->title,
                    'amount' => $contract->budget,
                    'formatted_amount' => 'R$ '.number_format((float) $contract->budget, 2, ',', '.'),
                ],
                'read_at' => null,
            ]);
    ...
    public static function notifyCreatorOfPaymentReceived($contract): void
    {
        try {
            $notification = Notification::create([
                ...
                'message' => "O pagamento do contrato '{$contract->title}' foi recebido. Valor: R$ ".number_format((float) $contract->creator_amount, 2, ',', '.'),
                'data' => [
                    'contract_id' => $contract->id,
                    'contract_title' => $contract->title,
                    'amount' => $contract->creator_amount,
    ...
    public static function notifyUserOfPaymentCompleted($jobPayment): void
    {
        try {
            $notification = Notification::create([
                ...
                'message' => 'Seu pagamento de R$ '.number_format((float) $jobPayment->creator_amount, 2, ',', '.').' foi processado com sucesso.',
                'data' => [
                    'job_payment_id' => $jobPayment->id,
                    'contract_id' => $jobPayment->contract_id,
                    'amount' => $jobPayment->creator_amount,
                    'formatted_amount' => 'R$ '.number_format((float) $jobPayment->creator_amount, 2, ',', '.'),
    ...
    public static function notifyUserOfPaymentRefunded($jobPayment, ?string $reason = null): void
    {
        try {
            $notification = Notification::create([
                ...
                'message' => 'O pagamento de R$ '.number_format((float) $jobPayment->creator_amount, 2, ',', '.').' foi estornado.'.($reason ? " Motivo: {$reason}" : ''),
                'data' => [
                    'job_payment_id' => $jobPayment->id,
                    'contract_id' => $jobPayment->contract_id,
                    'amount' => $jobPayment->creator_amount,
