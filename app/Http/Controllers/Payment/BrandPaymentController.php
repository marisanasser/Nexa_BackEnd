...
class BrandPaymentController extends Controller
{
    ...
    public function savePaymentMethod(StoreBrandPaymentMethodRequest $request): JsonResponse
    {
        ...
        try {
            $paymentMethod = $this->paymentService->saveBrandPaymentMethod($user, $request->validated());
    ...
    public function createCheckoutSession(Request $request): JsonResponse
    {
        try {
            ...
            $session = $this->paymentService->createSetupCheckoutSession($user, $successUrl, $cancelUrl);
    ...
    public function createFundingCheckout(Request $request): JsonResponse
    {
        try {
            ...
            $chatRoom = ChatRoom::where('room_id', $request->chat_room_id)->first();
    ...
    public function handleOfferFundingSuccess(Request $request): JsonResponse
    {
        try {
            ...
            $existingTransaction = Transaction::where('stripe_payment_intent_id', $paymentIntentId)
    ...
    public function checkFundingStatus(Request $request): JsonResponse
    {
        try {
            ...
            $hasFunding = Transaction::where('user_id', $user->id)
                ->where('status', 'paid')
            ...
            $brandBalance = BrandBalance::where('brand_id', $user->id)->first();
