...
use App\Events\Offer\OfferRejected;
...
class OfferController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        if ($response = $this->ensurePaymentMethod($user, $request)) {
            return $response;
        }

        $validationResult = $this->validateOfferContext($user, $request);
        if ($validationResult instanceof JsonResponse) {
            return $validationResult;
        }

        [$creator, $chatRoom] = $validationResult;

        return $this->performOfferCreation($user, $creator, $chatRoom, $request);
    }
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $paginator = $query->with(['brand:id,name,avatar_url', 'creator:id,name,avatar_url'])
                ->orderBy('created_at', 'desc')
                ->paginate(10)
            ;

            $items = collect($paginator->items())->map(function ($offer) use ($user) {
    ...
    private function ensureStripeAccount(User $user, Request $request): ?JsonResponse
    {
    ...
    private function ensurePaymentMethod(User $user, Request $request): ?JsonResponse
    {
    ...
    private function performOfferCreation(User $user, User $creator, ChatRoom $chatRoom, Request $request): JsonResponse
    {
        try {
            $offer = Offer::create([
                'brand_id' => $user->id,
