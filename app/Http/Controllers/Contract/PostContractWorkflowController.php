...
class PostContractWorkflowController extends Controller
{
    public function getContractsWaitingForReview(): JsonResponse
    {
        ...
        try {
            ...
            $contracts->transform(fn ($contract) => [
                'id' => $contract->id,
                ...
                'has_review' => null !== $contract->review,
            ]);
    ...
    public function getWorkHistory(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $contracts->through(function ($contract) use ($type) {
                $otherUser = 'creator' === $type ? $contract->brand : $contract->creator;
    ...
    private function calculateWorkHistoryStats(User $user, string $type): array
    {
        ...
        $totalEarnings = 'creator' === $type ? $query->sum('creator_amount') : $query->sum('budget');
        ...
        return [
            ...
            'total_earnings' => 'creator' === $type ? 'R$ '.number_format($totalEarnings, 2, ',', '.') : 'R$ '.number_format($totalEarnings, 2, ',', '.'),
